...
def add_item(current_cart, items_to_add):
    ...
    """
    for item, count in Counter(items_to_add).items():
...
def send_to_store(cart, aisle_mapping):
    ...
    """
    common_items = cart.keys() & aisle_mapping.keys()
    return {
        item: [cart[item], *aisle_mapping[item]]
        for item in sorted(common_items, reverse=True)
    }
...
def update_store_inventory(fulfillment_cart, store_inventory):
    ...
    for item, data in fulfillment_cart.items():
        inventory_item = store_inventory.get(item)
        if inventory_item is not None:
            inventory_item[0] -= data[0]
